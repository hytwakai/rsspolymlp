...
class SortStructure:
    ...
    def __init__(self):
        ...
        self.gval_str = []
        self.errors = Counter()
        self.error_poscar = defaultdict(list)
        self.time_all = 0

    def read_and_process(self):
        """Read POSCAR files, process results, and filter based on convergence criteria."""
        self.poscar_paths = sorted(glob.glob("initial_str/*"), key=lambda x: int(x.split("_")[-1]))

        for pos in self.poscar_paths:
            poscar_name = os.path.basename(pos)
            logfile = f"log/{poscar_name}.log"
            ...
            # Verify if the optimized structure file exists
    ...
    def process_optimized_structure(self, _res, optimized_path):
        ...
        _res["distance"] = GenerateInitialStructure().nearest_neighbor_atomic_distance(
    ...
    def update_nondup_structure(self, _res):
        """Check for duplicates in nondup_str and update or add new structures."""
        app_ = True
        change = False
    ...
    def extract_spg_count(self, spg_list):
        """Calculate the sum of space group counts from a list of space group strings."""
        return sum(
            int(re.search(r"\((\d+)\)", s).group(1)) for s in spg_list if re.search(r"\((\d+)\)", s)
        )

    def update_iteration_stats(self, _res):
        """Update iteration-related statistics such as iter_str, fval_str, and gval_str."""
    ...
    def add_new_structure(self, _res):
        """Append a new structure to nondup_str and update iteration statistics."""
    ...
    def run_sorting(self):
        ...
        with open("sorted_result.log", "w") as f:
            ...
            print("Number of initial structures:  ", len(self.poscar_paths), file=f)
            print("Number of optimized structures:", len(self.poscar_paths) - error_count, file=f)
